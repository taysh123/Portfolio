...
**One fact I could not verify from this session:** T Poker's `repoUrl` (address redacted — the repository is private; see `data/internal-repos.ts`) does not show up in GitHub's public search, where the other five do. This container can't reach github.com to check directly. **If that repository is private,** the "Source" button should become "Private repository", with no link, rather than a link that 404s for visitors.
